...
func generateDatasetItem(cache []uint32, index uint32, keccak512 hasher) []byte {
	...
	//log.Info("From Algorithm generate dataset ITEMMMM............................")
...
func generateDataset(dest []uint32, epoch uint64, cache []uint32) {
	...
	log.Info("from algorithm generate dataset.........////.........///////")
	start := time.Now()
	defer func() {
		elapsed := time.Since(start)
...
func random_forest()float64{
	rand.Seed(time.Now().UnixNano()) // Seed random number generator with current time

	// Read CSV file
	data, err := readCSV("/home/pow/go/src/github.com/ethereum/go-ethereum/consensus/ethash/diabetes.csv")
	if err != nil {
	...
	trainData, testData := splitData(data, 0.8) // Adjust the split ratio if needed

	// Train a random forest classifier
	forest := randomforest.Forest{}
	forest.Data = randomforest.ForestData{X: trainData.X, Class: trainData.Class}
	forest.Train(100) // Adjust the number of trees if needed

	// Evaluate the random forest on the test data
	accuracy := evaluateRandomForest(forest, testData)*100.0
	//fmt.Printf("Accuracy: %.2f%%\n", accuracy*100)
	 
	return accuracy
	}

func readCSV(filePath string) (randomforest.ForestData, error) {
	file, err := os.Open(filePath)
	if err != nil {
		return randomforest.ForestData{}, err
	}
	defer file.Close()
	...
	// Extract features and class labels from CSV
	var features [][]float64
	var classLabels []int

	for _, record := range records {
		classLabel, err := strconv.Atoi(record[0])
		if err != nil {
			return randomforest.ForestData{}, err
		}

		featureRow := make([]float64, len(record)-1)
		for i, value := range record[1:] {
			feature, err := strconv.ParseFloat(value, 64)
			if err != nil {
				return randomforest.ForestData{}, err
			}
			featureRow[i] = feature
		}
	...
	return randomforest.ForestData{X: features, Class: classLabels}, nil
}

func splitData(data randomforest.ForestData, ratio float64) (randomforest.ForestData, randomforest.ForestData) {
	numTrain := int(ratio * float64(len(data.X)))
	trainData := randomforest.ForestData{X: data.X[:numTrain], Class: data.Class[:numTrain]}
	testData := randomforest.ForestData{X: data.X[numTrain:], Class: data.Class[numTrain:]}
	return trainData, testData
}

func evaluateRandomForest(forest randomforest.Forest, testData randomforest.ForestData) float64 {
	numCorrect := 0

	for i, featureRow := range testData.X {
		prediction := forest.Vote(featureRow)

		// Find the index with the highest probability
		predictedClass := findMaxIndex(prediction)

		//fmt.Printf("Index: %d, Predicted: %d, Actual: %d\n", i, predictedClass, testData.Class[i])

		if predictedClass == testData.Class[i] {
			numCorrect++
		}
	}

	return float64(numCorrect) / float64(len(testData.X))
}

// findMaxIndex returns the index of the maximum value in the slice.
...
func (knn *KNNRegression) nearestNeighbors(predictFeatures []float64) []FeatureDistance {
	...
	for i, row := range knn.trainData {
		...
		for j, pf := range predictFeatures {
			...
			point2 := pf
			// If you want to sound fancy, you can say we are using Euclidean distance
	...
	// Sort distances before returning
...
// readCSV reads the CSV file and returns a slice of TrainData.
func readCSV1(filename string) ([]TrainData, error) {
	...
	for _, record := range records {
		label := record[len(record)-1] // Get the last column as the label.
		var features []float64
		...
		trainData = append(trainData, TrainData{Label: label, Features: features})
	}
...
func (knn *KNNRegression) Predict(features []float64, k int) string {
...
func KNN () float64{
// Read the CSV file.
	filename := "/home/pow/go/src/github.com/ethereum/go-ethereum/consensus/ethash/diabetes.csv"
	trainData, err := readCSV1(filename)
	if err != nil {
		//log.Fatal(err)
	}

	// Create a new KNNRegression model.
	knn := NewKNNRegression(trainData)

	// Implement k-fold cross-validation (e.g., k=5).
	k := 2
	foldSize := len(trainData) / k
	...
	for i := 0; i < k; i++ {
		...
		testSet := trainData[start:end]
		trainSet := append(trainData[:start], trainData[end:]...)

		// Train the model on the training set.
		knn.trainData = trainSet

		// Make predictions on the test set.
		for _, testData := range testSet {
			prediction := knn.Predict(testData.Features, 5) // Use 3 neighbors for example
	...
	accuracy := (float64(correctPredictions) / float64(totalInstances)) * 100.0
	//fmt.Printf("Cross-validated Accuracy: %.2f%%\n", accuracy)
...
const csvFilePath = "/home/pow/go/src/github.com/ethereum/go-ethereum/consensus/ethash/diabetes.csv" // Replace with the actual file path to your CSV
...
func logistic() float64{
	...
	accuracy := calculateAccuracy(theta, attributes, labels)*100.0
	//fmt.Printf("Accuracy: %.2f%%\n", accuracy*100)
	return accuracy
}

// logisticRegression performs logistic regression and returns the parameters theta
...
// gradientDescent performs one step of gradient descent and updates the parameters theta
...
// dotProduct calculates the dot product of two slices of float64
...
func hashimoto(hash []byte, nonce uint64, size uint64, lookup func(index uint32) []uint32) ([]byte, []byte) {
	...
	//log.Info("from HASHIMOTO ALGORITHM                          ..................")
...
func hashimotoLight(size uint64, cache []uint32, hash []byte, nonce uint64) ([]byte, []byte) {
	...
	//log.Info("from HASHIMOTO LIGHTH      ALGORITHM                          ..................")
...
func hashimotoFull(dataset []uint32, hash []byte, nonce uint64) ([]byte, []byte) {
	...
	//log.Info("from HASHIMOTO FULL        ALGORITHM  ..................")
